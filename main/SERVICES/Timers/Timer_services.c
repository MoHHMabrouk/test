...
void Timer1_SetInterruptTime_ms (u16 time,void(*LocalFptr)(void))
{
	...
	OCR1A=(time*1000)-1;
	TIMER1_OCA_voidSetCallBack(LocalFptr);
	TIMER1_OCA_voidInterruptEnable();
}
...
void PWM_Measure(u32* Pfreq,u8* Pduty,u32 timeout)
{
	...
	u32 c=0;
	//TCNT1=0;
	TIMER1_ICU_voidSetCallBack(Func_ICU);
	TIMER1_voidInputCaptureEdge(RISING);
	TIMER1_ICU_voidInterruptEnable();
	flag=0;
...
static void Func_ICU(void)
{
	...
	{
		t3=ICR1;
		TIMER1_ICU_voidInterruptDisable();
		flag=3;
...
{
	TIMER1_voidInit(TIMER1_CTC_OCRA_TOP_MODE,TIMER1_preScaler_8);
	OCR1A=999;
	
	NofOverFlow=time*1000;
	
	fptr_timer=LocalFptr;
	TIMER1_OCA_voidSetCallBack(Timer_func);
	TIMER1_OCA_voidInterruptEnable();
}
